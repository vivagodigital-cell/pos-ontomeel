...
try {
    if ($action === 'getItems') {
        ...
        $stmt_b = $pdo->query("SELECT b.*, COALESCE(c.name, b.item_type, 'Books') as category_name, COALESCE(c.name, b.item_type, 'Books') as item_type, 'books' as source_table 
                               FROM books b 
        ...
        $stmt_i = $pdo->query("SELECT i.id, i.item_name as title, COALESCE(c.name, 'General') as category_name, COALESCE(c.name, 'General') as item_type, i.item_type as category_id, i.quantity as stock_qty, i.sell_price, i.unit_cost as purchase_price, i.supplier_name, i.barcode as isbn, i.is_active, '' as cover_image, '' as author, 'inventory_items' as source_table 
        ...
        // Resolve category_id if missing but type name is present
        if (!$category_id && !empty($type)) {
            $getCat = $pdo->prepare("SELECT id FROM categories WHERE name = ?");
            $getCat->execute([$type]);
            $category_id = $getCat->fetchColumn() ?: null;
        }

        if ($action === 'addItem') {
            if (strtolower($type) === 'books' || strtolower($type) === 'book' || $category_id == 1) { // Assuming 1 is Books
                ...
                $stmt->execute([$title, $category_id, $stock, $cost, $price, $supplier_name, $isbn]);
            }
            ...
            if ($source_table === 'inventory_items') {
                ...
                $stmt->execute([$title, $category_id, $stock, $cost, $price, $supplier_name, $isbn, $id]);
            } else {
